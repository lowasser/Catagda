...
record Iso {ℓA ℓB : Level} {A : Set ℓA} {B : Set ℓB} {{SA : Setoid A}} {{SB : Setoid B}} (f : A → B) (g : B → A) : Set (ℓA ⊔ ℓB) where
    field
...
_⁻¹ : {ℓA ℓB : Level} {A : Set ℓA} {B : Set ℓB} → {{SA : Setoid A}} → {{SB : Setoid B}} → { f : A → B } {g : B → A} → Iso f g → Iso g f
iso ⁻¹ = record {
...
private
    ...
    iso-injective : {ℓA ℓB : Level} {A : Set ℓA} {B : Set ℓB} {{SA : Setoid A}} {{SB : Setoid B}} → { f : A → B } { g : B → A } → Iso f g → Injective f
    iso-injective {_} {_} {A} {B} {f} {g} iso {a1} {a2} fa1≈fa2 = begin≈
...
iso-is-injective : {ℓA ℓB : Level} {A : Set ℓA} {B : Set ℓB} {{SA : Setoid A}} {{SB : Setoid B}} → { f : A → B } { g : B → A } → Iso f g → IsInjective f
iso-is-injective iso = record { injective = iso-injective iso }

private 
    surjective-reverse-compose-is-identity : {ℓA ℓB : Level} {A : Set ℓA} {B : Set ℓB} → {{SA : Setoid A}} → {{SB : Setoid B}} →
    ...
    injective-surjective-compose-is-identity : {ℓA ℓB : Level} {A : Set ℓA} {B : Set ℓB} → {{SA : Setoid A}} → {{SB : Setoid B}} →
    ...
    injective-surjective-preimage-is-congruent : {ℓA ℓB : Level} {A : Set ℓA} {B : Set ℓB} → {{SA : Setoid A}} → {{SB : Setoid B}} →
        (f : A → B) → (surjf : Surjective f) → (injf : Injective f) → Congruent (preimage f surjf)
    injective-surjective-preimage-is-congruent {_} {_} {_} {B} f surjf injf {b1} {b2} b1≈b2 = injf lemma3
...
injective-surjective-to-iso : {ℓA ℓB : Level} {A : Set ℓA} {B : Set ℓB} → {{SA : Setoid A}} → {{SB : Setoid B}} →

...
open import Definitions.List.Setoid {ℓA} {ℓ=A} A renaming (nil=[]=nil to Anil=nil; cons=[]=cons to Acons=cons)
open import Definitions.List.Setoid {ℓB} {ℓ=B} B renaming (nil=[]=nil to Bnil=nil; cons=[]=cons to Bcons=cons)
open Setoid {{...}}

map-congruent : (f : A → B) → {{IC : IsCongruent f}} → {a1 a2 : [ A ]} → a1 ≅ a2 → map f a1 ≅ map f a2
map-congruent f Anil=nil = Bnil=nil
